...
// require("./auth");
import './auth.js'; // Ensure Passport strategy setup
...
app.get('/auth/google/callback',
  passport.authenticate('google', {
    ...
    session: false // or true if you use sessions
  }),
  (req, res) => {
    // Success! Redirect or respond with token/session
    const token = generateJWT(req.user); // If using JWT
...
// Connect to MongoDB
mongoose.connect('mongodb://localhost:27017/PatientRegistration')
...
//                     doctorname: user.fullname // or `user.doctorname` if it's stored that way
//                 });
//             });
//         } else {
...
app.post("/login", async (req, res) => {
    ...
    try {
        ...
        // Login success
        const redirectUrl = user.role === 'doctor' ? '/doctorhome' : '/home';
...
// Fetch all doctor profiles(testing)
app.get('/api/getDoctorProfiles', async (req, res) => {
    try {
...
app.post('/api/updateDoctorRating', async (req, res) => {
    ...
    try {
        ...
        res.json({ message: 'Rating updated successfully', doctor });
    } catch (error) {
        console.error('Error updating doctor rating:', error);
        res.status(500).json({ error: 'Failed to update rating' });
    }
});
// TESTING --------------------------------(for the patient data)
...
const PatientSchema = new mongoose.Schema({
  ...
  // API Endpoint to create a patient profile
  app.post('/api/patientProfiles', upload.single('profilePhoto'), async (req, res) => {
    try {
        ...
        // Respond with success and the generated patient ID
        res.status(201).json({ message: 'Profile saved successfully', patientId });
    } catch (error) {
        console.error(' creating patient profile:', error);
